...
/// Check if recording is allowed based on onboarding status.
/// Cloud transcription needs no local model, so recording is available
/// whenever onboarding is complete.
