...
pub async fn stop_recording<R: Runtime>(
    ...
    // Step 3: No local model to unload — cloud transcription providers hold no
    // local resources, so shutdown proceeds directly to analytics/cleanup.
    info!("🧠 All transcript chunks processed. Cloud provider holds no local model; skipping unload step.");
