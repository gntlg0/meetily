...
pub async fn start_retranscription<R: Runtime>(
    ...
    let result = run_retranscription(app.clone(), meeting_id.clone(), meeting_folder_path, language).await;
...
async fn run_retranscription<R: Runtime>(
    ...
    info!(
        "Starting retranscription for meeting {} with language {:?}",
        meeting_id, language
    );
    ...
    emit_progress(&app, &meeting_id, "transcribing", 25, "Preparing cloud transcription...");
