...
async fn run_import<R: Runtime>(
    ...
    info!(
        "Starting import for '{}' from {} with language {:?}",
        title, source_path, language
    );
    ...
    emit_progress(&app, "transcribing", 30, "Preparing cloud transcription...");
    ...
    if processable_count > 0 && !asr.has_api_key() {
        return Err(anyhow!(
            "No API key configured for '{}'. Add it in Settings → Transcription, then import again.",
            asr.provider_id()
        ));
    }
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        // Transcribe via the cloud ASR API
